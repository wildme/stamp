...
const EditDirective = () => {
  ...
  function deleteFile(file) {
    const url = `/api/directive/attachment/delete/${file}`;
    const token = localStorage.getItem('at');
    return fetch(url, {
      method: 'DELETE',
      headers: {'Authorization': `Bearer ${token}`}
  ...
  function saveRecord(fileProps) {
    ...
    const url = `/api/directive/update/${id}`;
    const token = localStorage.getItem('at');
    setDisableBtn(true);
    fetch(url, {
      method: 'PUT',
      ...
      body: JSON.stringify({subject, note, fileData, owner})
    })
      .then(res => {
        if (res.status === 200) {
  ...
  useEffect(() => {
    ...
    const url = `/api/edit/directive/${id}`;
    const token = localStorage.getItem('at');
