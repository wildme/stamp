...
const DirectiveCard = () => {
  ...
  const [typeCode, setTypeCode] = useState('Loading...');
  ...
  const handleStatus = () => {
    ...
    const url = `/api/directive/status/${id}`;
  ...
  useEffect(() => {
    ...
    const url = `/api/view/directive/${id}`;
    const token = getTokenFromLocalStorage();
    ...
    fetch(url, { headers: { 'Authorization': `Bearer ${token}` }, signal })
      ...
      .then(data => {
        ...
        if (data !== 1) {
          ...
          setTypeCode(data.record.typeCode);
          setUpdated(data.record.updatedAt);
          setStatus(data.record.status);
          setDate(data.record.createdAt);
    ...
    <div className="record-card-grid">
      ...
      <div className="record-card">
        <div className="record-card__title">
          {<h2>{t('recordCard.titleDirective')} #{idOfRec}-{typeCode}</h2>}
          <hr/>
        </div>
        ...
        {file?.fsName &&
          <div className="record-card__file record-card__file_long">
            <DownloadLink
              api={`/api/directive/download/${file.fsName}`}
              linkname={file.name}
              hash={file.fsName}
              filename={file.name}
            />
